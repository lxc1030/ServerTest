...
public class KCPPlayer
{
    ...
    public void Init(IPEndPoint localPoint, IPEndPoint remotePoint, KCPReceiveListener listen)
    {
        ...
        m_Socket.AddReceiveListener(remotePoint, listen);

        NetworkDebuger.Log(LOG_TAG, " 初始化。");
    }
    ...
    public void SendMessage(IPEndPoint remotePoint, byte[] sendBytes)
    {
        if (m_Socket != null)
        {
            //m_MsgId++;
            m_Socket.SendTo(sendBytes, sendBytes.Length, remotePoint);
        }
    }
}