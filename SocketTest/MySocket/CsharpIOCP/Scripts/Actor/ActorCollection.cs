...
public class ActorCollection
{
    ...
    protected Dictionary<Guid, int> GuidUniqueID { get; set; }              // 從Guid取得會編號
    ...
    public ActorReturn ActorOnline(Guid guid, int UniqueID, string MemberID, string Nickname, short Sex)
    {
        ...
        lock (this)
        {
            ...
            else
            {
                ...
                Actor actor = new Actor(guid, UniqueID, MemberID, Nickname, Sex); // 加入會員列表
    ...
    // 以會員編號取得會員資料
    public Actor GetActor(int UniqueID)
    {
        Actor actor;
